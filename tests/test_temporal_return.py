...
class TemporalReturnTests(unittest.TestCase):
    def test_different_encounters_same_anchor_selected_changed_axis(self):
    ...
    def test_absent_witness_does_not_become_agreement(self):
        sample = specimen()
    ...
    def test_separately_declared_sidereal_conventions_do_not_mix(self):
        sample = specimen()
    ...
    def test_rejects_occurrence_timestamp_mismatch(self):
        sample = specimen()
        sample["encounters"][1]["temporalWitness"]["instant_utc"] = "2026-10-19T14:00:00Z"
        with self.assertRaisesRegex(TemporalReturnError, "witness instant") as exc:
            compare_temporal_return(sample)
        assert exc.exception.code == "TIMESTAMP_MISMATCH"


    def test_rejects_same_id_and_changed_anchor(self):
        sample = specimen()
        sample["encounters"][1]["encounterId"] = "enc-a"
        with self.assertRaises(TemporalReturnError) as exc:
            compare_temporal_return(sample)
        assert exc.exception.code == "DUPLICATE_OCCURRENCE"
        sample = specimen()
        sample["encounters"][1]["anchor"]["scriptureRef"]["verse"] = 2
        with self.assertRaises(TemporalReturnError) as exc:
            compare_temporal_return(sample)
        assert exc.exception.code == "ANCHOR_MISMATCH"
    ...
    def test_rejects_invalid_coordinate(self):
        ...
        with self.assertRaises(TemporalReturnError) as exc:
            compare_temporal_return(sample)
        assert exc.exception.code == "INVALID_COORDINATE"


    def test_rejects_wrong_frame_and_backward_time(self):
        sample = specimen()
        ...
        with self.assertRaises(TemporalReturnError) as exc:
            compare_temporal_return(sample)
        assert exc.exception.code == "REVERSED_TIME"


    def test_distinct_source_witnesses_same_scripture_address(self):
        sample = specimen()
    ...
    def test_ayanamsa_offset_can_evolve_between_dates_within_same_convention(self):
        sample = specimen()
