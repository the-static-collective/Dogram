...
def test_zero_jet_claim_refuses_hidden_limit_assumption():
    try:
        zero_taylor_jet(3, limit_fact_declared=False)
    except ValueError as exc:
        assert "flatness limit theorem" in str(exc)
    else:
        raise AssertionError("expected explicit proof-obligation refusal")
